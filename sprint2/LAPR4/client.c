...
void view_cell(sharedboard *board,
...
void add_image(sharedboard *board,
               sem_t *sems[BOARD_ROWS][BOARD_COLS][CELL_SEM_LAST]) {
  size_t row, col;
...
void add_text(sharedboard *board,
              sem_t *sems[BOARD_ROWS][BOARD_COLS][CELL_SEM_LAST]) {
  size_t row, col;
  char buf[256];
...
void swap_cells(sharedboard *board,
                sem_t *sems[BOARD_ROWS][BOARD_COLS][CELL_SEM_LAST]) {
  size_t row1, col1;
  size_t row2, col2;
  ...
  if (sem_timedwait(cell_a, &spec) == -1) {
    ...
    printf("Swapped cells (%zu, %zu) and (%zu, %zu) with success!", row1, col1,
           row2, col2);
  }
}

void *init_mem(const char *restrict name, size_t size) {
  int fd;
  void *block;
...
size_t read_option(const char *restrict fmt, ...) {
  char buf[BUFSIZ];
  va_list ap;
...
char *query_boards(const boardlist *list, sem_t *board_sem) {
  size_t i, len;
  char *chosen = NULL;
...
static void init_sems(const char *board_name,
                      sem_t *sems[BOARD_ROWS][BOARD_COLS][CELL_SEM_LAST],
  ...
  for (i = 0; i < rows; i++) {
    for (j = 0; j < cols; j++) {
      int ret =
          snprintf(buf, sizeof(buf), "%s.%zu.%zu.mutex", board_name, i, j);
